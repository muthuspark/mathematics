...
**Define the Inverse Cosine Functions**

Let:

$$\theta = \cos^{-1}(x)$$This implies:$$\cos(\theta) = x$$where$0 \leq \theta \leq \pi$because the range of$\cos^{-1}$is$[0, \pi]$.

**Express the Negative Value in Terms of Cosine**

We need to find $\phi$such that:$$\phi = \cos^{-1}(-x)$$This means:$$\cos(\phi) = -x$$

**Utilize the Trigonometric Identity for Supplementary Angles**

Recall that:

$$\cos(\pi - \theta) = -\cos(\theta)$$Substituting$\cos(\theta) = x$into this identity:$$\cos(\pi - \theta) = -x$$

**Equate the Angles**

Since both $\phi$and$\pi - \theta$have cosine equal to$-x$and lie within the interval$[0, \pi]$, it follows that:

$$\phi = \pi - \theta$$**Substitute Back for$\theta$**

From Step 1, we know that $\theta = \cos^{-1}(x)$. Therefore:

$$\phi = \pi - \cos^{-1}(x)$$But from Step 2,$\phi = \cos^{-1}(-x)$. Hence:
