...
$$(1 + x)^n = \sum_{k=0}^{n} C(n, k) \cdot x^k$$where$C(n, k)$is the binomial coefficient, calculated as:$$C(n, k) = \frac{n!}{k!(n - k)!}$$
...
$$(1 + x)^0 = 1$$- If$n = 1$:
$$(1 + x)^1 = 1 + x$$- If$n = 2$:
$$(1 + x)^2 = 1 + 2x + x^2$$Each term in the expansion follows the pattern of increasing powers of$x$ with corresponding binomial coefficients.

### Extension to Negative Exponents

When $n$is negative, such as$-1$, the expansion becomes an infinite series. For example:
$$(1 + x)^{-1} = 1 - x + x^2 - x^3 + \dots$$This geometric series converges for$|x| < 1$.
